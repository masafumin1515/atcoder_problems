...
**考察**
- 幅優先っぽい
- queueに次の座標突っ込んでいけば解けそう

一つのマスを白->黒に変える回数は計算できそうだが、「何回の操作でできるか」の求め方がわからなかった。
言い換えると、「黒のマスから一回の操作で探索できる範囲を探索したら、+1」なので

- 二つのqueueを用意する。

が解決策。
**反省**
- O(HW)なら間に合う->全てのマスを一回しか探索してはいけないと言い換える。
- 全てのマスを一回しか探索できないなら、黒から白を探す方法はないかと考える。
- 一回の操作で左右上下に1しか探索できない条件から、幅優先になるとわかる。


あと配列だとarr\[x\]\[y\]じゃなくてarr\[y\]\[x\]。毎回バグる。
多点スタート問題としても解けるらしい。
...
**考察**
- 中心座標(cx,cy)がわかれば、ある一点の情報のみから高さも計算できる。
- 求めた高さが全ての点で適合しているか確認すれば良い？
